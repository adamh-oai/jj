...
pub struct SnapshotOptions<'a> {
    ...
    /// Older fingerprints accepted only while advancing an already-committed
    /// AWACS baseline to the current interpretation model.
    pub awacs_compatible_input_fingerprints: Vec<[u8; 32]>,
}
