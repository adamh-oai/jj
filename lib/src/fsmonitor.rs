...
/// Canonical inputs consulted while interpreting an AWACS snapshot scan.
///
/// Snapshot-backed working copies keep the tracked/untracked classification
/// from their committed baseline for paths that AWACS did not report as
/// changed. Consequently, ignore and auto-tracking policy changes affect only
/// newly changed paths and do not invalidate the semantic baseline.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl AwacsInputFingerprintV1 {
    ...
    /// Computes the domain-separated SHA-256 fingerprint for inputs that can
    /// reinterpret unchanged tracked paths.
    pub fn sha256(&self) -> [u8; 32] {
        let mut hasher = sha2::Sha256::new();
        hasher.update(b"jj:awacs-semantic-input-fingerprint:v2\0");
    ...
    /// Computes the original v1 fingerprint so an existing committed baseline
    /// can advance once onto the sticky-classification v2 model without a
    /// replacement full scan.
    pub fn legacy_sha256(&self) -> [u8; 32] {
        let mut hasher = sha2::Sha256::new();
...
mod tests {
    ...
    fn awacs_fingerprint_changes_for_external_input_changes() {
        ...
        assert_eq!(inputs.sha256(), changed.sha256());
        assert_ne!(inputs.legacy_sha256(), changed.legacy_sha256());

        let mut missing = inputs.clone();
        missing.core_excludes.contents = None;
        assert_eq!(inputs.sha256(), missing.sha256());
        assert_ne!(inputs.legacy_sha256(), missing.legacy_sha256());
    }

    #[test]
    fn awacs_semantic_fingerprint_ignores_tracking_policy_changes() {
        let inputs = fingerprint_inputs();
        let mut changed = inputs.clone();
        changed.snapshot_auto_track = b"none()".to_vec();
        changed.fileset_aliases.clear();
        changed.max_new_file_size = 0;
        assert_eq!(inputs.sha256(), changed.sha256());
        assert_ne!(inputs.legacy_sha256(), changed.legacy_sha256());

        changed.eol_conversion = b"none".to_vec();
