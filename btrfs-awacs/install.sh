...
if [ "$#" -gt 1 ]; then
  ...
  cargo build --profile "$1" -p jj-cli -p btrfs-awacs
  case "$1" in
    dev | test) build_dir=debug ;;
    release | bench) build_dir=release ;;
    *) build_dir=$1 ;;
  esac
else
...
install -m 0755 "$target_dir/$build_dir/jj" "$cargo_bin_dir/jj"
run_as_root install -d -m 0755 "$binary_dir"
run_as_root install -m 0755 "$target_dir/$build_dir/awacs" "$binary_dir/awacs"
