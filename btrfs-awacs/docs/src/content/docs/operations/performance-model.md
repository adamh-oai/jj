...
| Retained history and snapshots | Bounded by configured policy and pins | A periodic bounded worker now enforces retention and GC; sustained kernel-backed latency/recovery acceptance remains pending. |
...
The dominant existing costs are not subtle micro-optimizations:
filesystem-wide flushes, repeated kernel comparisons, full-tree crawls on
every edit, and globally serialized leases can dominate the intended benefit
of a filesystem monitor.
