...
| Database bootstrap | `src/store.rs` | Create/open manager and broker SQLite databases, configure WAL/foreign keys, extract the normative SQL schema from the design document, migrate, and load cursor-key metadata. |
...
| Cursor and path projection | `src/compat.rs` | Authenticate opaque direct cursors, project semantic events, and provide the precision-aware range projector. |
| Client-visible snapshot facade | `src/facade.rs` | Activate a monitored view, verify continuity, request cuts, resolve exact historical baselines, mint cursors, pin response inputs, and release/renew query leases. |
...
| Executable and activation | `src/main.rs` | Provide CLI entry points, start/discover the namespace daemon, configure broker/state/snapshot paths, publish the scan socket, authenticate clients, and dispatch connections. |
...
| Monitor settings and fingerprint | `../jj/lib/src/fsmonitor.rs` | Parse `none` and `awacs`; represent the AWACS socket/client; compute versioned, canonical SHA-256 external-input fingerprints. |
...
| Persisted monitor cursors | `../jj/lib/src/protos/local_working_copy.proto` | Store a backend-tagged `FsmonitorCursor` with the direct AWACS cursor and its input fingerprint. |
...
The direct backend must be optional and must not alter existing `none`
behavior. Several current deviations from that requirement are documented
below.
