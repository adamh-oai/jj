...
use crate::broker::{
    ChangedObjectsExecution, ChangedObjectsResult, ExpectedSubvolume,
    INODE_PATH_OUTPUT_HEADER_SIZE, INODE_PATH_OUTPUT_MAGIC, INODE_PATH_OUTPUT_VERSION,
    InodePathsExecution, MAX_CHANGED_OBJECT_OUTPUT, MAX_INODE_PATH_OUTPUT,
    NestedSubvolumesExecution, SeqPacket, SnapshotTrashExecution,
};
use crate::broker_protocol::{BrokerClient, BrokerDispatcher};
use crate::btrfs::{
    ChangedObjectsIoctlResult, OpenedSubvolume, ROOT_INODE,
    create_snapshot as create_btrfs_snapshot,
};
use crate::index::{Object, derive_events_from_endpoint_paths};
use crate::manager::{
    ...
    SnapshotIdentity, append_replay_spool_payload,
};
use crate::manifest::{
    CHANGE_CREATED, CHANGE_XATTR, CHANGED_OBJECTS_V2_MAGIC, ChangedObjectsManifest,
...
use crate::tree_index::{PRIVILEGE_FSCRYPT, materialize_stream_object};
...
/// Private capability proving that every filesystem location needed by a new
/// physical snapshot effect was writable through this manager's mount view.
struct SnapshotLifecycleWriteAccess;

enum InitialSnapshotSource {
    Ordinary,
    InheritedRevision {
...
pub struct ServiceConfig {
    ...
    /// Stable broker-owned trash directory. Its parent stays manager-owned so
    /// the manager can pass an fd, while the broker creates and owns this
    /// child and drains it after each restart.
    pub snapshot_trash_directory: PathBuf,
...
impl ServiceConfig {
    pub fn new(
        ...
        let snapshot_trash_directory = managed_snapshot_directory.join(".broker-trash-default");
        Self {
            managed_snapshot_directory,
            snapshot_trash_directory,
            spool_directory,
            boot_id,
    ...
    pub fn with_snapshot_trash_directory(mut self, path: PathBuf) -> Self {
        self.snapshot_trash_directory = path;
        self
    }
...
impl Service {
    ...
    pub fn new(mut store: Store, config: ServiceConfig) -> Result<Self, ServiceError> {
        ...
        let dispatcher = BrokerDispatcher::new(manager_uid)
            .map_err(|error| ServiceError::context("start broker snapshot trash", error))?;
        ...
        service.request_snapshot_trash_drain()?;
    ...
    pub fn new_external(store: Store, config: ServiceConfig) -> Result<Self, ServiceError> {
        Self::new_external_with_recovery(store, config, true)
    }

    /// Opens only the authenticated broker channel needed by an explicit
    /// from-scratch rebuild.
    ///
    /// Rebuild discards the old database, so it must not run schema recovery,
    /// operation replay, or quarantine policy from that database before it can
    /// move physical snapshot wrappers into broker-owned trash.
    pub(crate) fn new_external_rebuild_cleanup(
        store: Store,
        config: ServiceConfig,
    ) -> Result<Self, ServiceError> {
        Self::new_external_with_recovery(store, config, false)
    }

    fn new_external_with_recovery(
        mut store: Store,
        config: ServiceConfig,
        recover_state: bool,
    ) -> Result<Self, ServiceError> {
    ...
    fn snapshot_trash_parent_and_name(&self) -> Result<(File, Vec<u8>), ServiceError> {
        let parent = self
            .config
            .snapshot_trash_directory
            .parent()
            .ok_or_else(|| ServiceError::new("snapshot trash directory has no parent"))?;
        let name = self
            .config
            .snapshot_trash_directory
            .file_name()
            .ok_or_else(|| ServiceError::new("snapshot trash directory has no basename"))?
            .as_bytes()
            .to_vec();
        let parent = File::open(parent)
            .map_err(|error| ServiceError::context("open snapshot trash parent", error))?;
        Ok((parent, name))
    }

    fn request_snapshot_trash_drain(&self) -> Result<(), ServiceError> {
        let (trash_parent, trash_name) = self.snapshot_trash_parent_and_name()?;
        self.broker
            .drain_snapshot_trash(&trash_name, trash_parent.as_fd())
            .map_err(|error| ServiceError::context("queue snapshot trash recovery", error))
    }

    /// Proves that every directory needed after a cut snapshot is created is
    /// writable through the caller's current mount view.
    ///
    /// The broker receives these directory fds from the manager, so a
    /// read-only sandbox bind remains read-only even after the fd crosses the
    /// privilege boundary. Failing here keeps the SQLite reservation and the
    /// physical snapshot effect from getting ahead of later spool or trash
    /// writes.
    fn preflight_snapshot_lifecycle_writable(
        &self,
    ) -> Result<SnapshotLifecycleWriteAccess, ServiceError> {
        probe_writable_directory(
            &self.config.managed_snapshot_directory,
            "preflight writable managed snapshot directory",
        )?;
        probe_writable_directory(
            &self.config.spool_directory,
            "preflight writable snapshot spool directory",
        )?;
        let trash_parent = self
            ...
            .parent()
            .ok_or_else(|| ServiceError::new("snapshot trash directory has no parent"))?;
        probe_writable_directory(trash_parent, "preflight writable snapshot trash parent")?;
        Ok(SnapshotLifecycleWriteAccess)
    }

    /// Moves every retained managed snapshot into broker-owned trash before an
    /// explicit from-scratch rebuild removes this store. The manager database
    /// is still open here, so the ordinary authenticated broker session can
    /// authorize the same trash RPC used by normal GC.
    pub fn trash_managed_snapshots_for_rebuild(&self) -> Result<usize, ServiceError> {
        let (trash_parent, trash_name) = self.snapshot_trash_parent_and_name()?;
        let mut trashed = 0;
        self.trash_snapshot_wrappers_for_rebuild(
            &self.config.managed_snapshot_directory,
            &trash_parent,
            &trash_name,
            &mut trashed,
    ...
    fn trash_snapshot_wrappers_for_rebuild(
        &self,
        directory: &Path,
        trash_parent: &File,
        trash_name: &[u8],
        ...
        let source_parent = File::open(directory)
            .map_err(|error| ServiceError::context("open rebuild snapshot parent", error))?;
        let entries = fs::read_dir(directory)
            .map_err(|error| ServiceError::context("read rebuild snapshot parent", error))?;
        for entry in entries {
            let entry = entry
                .map_err(|error| ServiceError::context("read rebuild snapshot entry", error))?;
            let name = entry.file_name();
            if name.as_bytes().starts_with(b".broker-trash-") {
                continue;
            }
            let path = entry.path();
            let metadata = fs::symlink_metadata(&path)
                .map_err(|error| ServiceError::context("inspect rebuild state entry", error))?;
            if !metadata.file_type().is_dir() {
                continue;
            }
            if metadata.ino() == ROOT_INODE {
                return Err(ServiceError::new(format!(
                    "rebuild state contains unsupported direct snapshot {}; remove the discarded pre-wrapper state before retrying",
                    path.display()
                )));
            }
            let snapshot_path = path.join(std::ffi::OsString::from_vec(
                MANAGED_SNAPSHOT_CHILD_NAME.to_vec(),
            ));
            let snapshot_metadata = match fs::symlink_metadata(&snapshot_path) {
                Ok(metadata) => Some(metadata),
                Err(error) if error.kind() == io::ErrorKind::NotFound => None,
                Err(error) => {
                    return Err(ServiceError::context("inspect rebuild snapshot", error));
                }
            };
            if snapshot_metadata.as_ref().is_some_and(|metadata| {
                metadata.file_type().is_dir() && metadata.ino() == ROOT_INODE
            }) {
                let target = self.open_subvolume(&snapshot_path).map_err(|error| {
                    ServiceError::context("open rebuild managed snapshot", error)
                })?;
                let expected =
                ...
                let result = self
                    ...
                    .map_err(|error| {
                        ServiceError::context("trash rebuild managed snapshot", error)
                    })?;
                log::debug!(
                    "AWACS rebuild snapshot trash completed: path={} verify={:?} rename={:?}",
                    snapshot_path.display(),
                    Duration::from_nanos(result.verify_elapsed_ns),
                    Duration::from_nanos(result.rename_elapsed_ns),
                );
                *trashed = trashed
                    .checked_add(1)
                    .ok_or_else(|| ServiceError::new("rebuild snapshot trash count overflow"))?;
                continue;
            }
            self.trash_snapshot_wrappers_for_rebuild(&path, trash_parent, trash_name, trashed)?;
        }
        Ok(())
    }
        ...
        for operation in recovering {
            let recorded = if let Some(recorded) = operation.recorded {
                    ...
                    self.recover_existing_snapshot(&operation.reserved_path, source_uuid)?
                {
                    ...
                    let destination_name = managed_snapshot_entry_name(
                        &operation.reserved_path,
                        &self.config.managed_snapshot_directory,
                    )?;
                    let write_access = self.preflight_snapshot_lifecycle_writable()?;
                    self.create_snapshot(&write_access, &source, &destination_name, true)?
                };
            ...
            self.store
                ...
                .map_err(|error| {
                    ServiceError::context("publish recovered initial snapshot", error)
                })?;
        }
        Ok(())
    }
    ...
    pub fn initialize(
        ...
        self.initialize_with_snapshot_source(source_path, options, InitialSnapshotSource::Ordinary)
    }

    /// Creates the first child snapshot with an inherited baseline cursor.
    pub fn initialize_with_inherited_revision(
        ...
        let _ = seed_revision_id;
        self.initialize_with_snapshot_source(
            source_path,
            options,
            InitialSnapshotSource::InheritedRevision {
                inherited_baseline_snapshot_uuid,
            },
        )
    }

    fn initialize_with_snapshot_source(
        &mut self,
        source_path: &Path,
        options: &InitializeOptions,
        initial_source: InitialSnapshotSource,
    ) -> Result<InitializedWatch, ServiceError> {
        ...
        let destination_path =
            managed_snapshot_path(&self.config.managed_snapshot_directory, &destination_name);
        ...
        let snapshot = self.create_snapshot(&write_access, &source, &destination_name, true)?;
        log::debug!(
        ...
        // TODO: bootstrap must explicitly reject an initial snapshot that
        // already contains fscrypt once the broker exposes a bounded
        // bootstrap capability. Incremental changed-objects streams reject
        // fscrypt introductions before publication below.
        let inherited_baseline_snapshot_uuid = match initial_source {
            InitialSnapshotSource::Ordinary => None,
            InitialSnapshotSource::InheritedRevision {
                inherited_baseline_snapshot_uuid,
            } => inherited_baseline_snapshot_uuid,
        };
        self.store
            ...
            .map_err(|error| ServiceError::context("publish initial snapshot", error))
    }
    ...
    pub fn changes(&mut self, options: &ChangesOptions) -> Result<PublishedCut, ServiceError> {
        ...
        let target = self.create_snapshot(&write_access, &live, &destination_name, true)?;
        tracing::info!(
    ...
    pub fn historical_changes(
        ...
        let (base_paths, target_paths) = self.resolve_changed_endpoint_paths(
            &parsed.manifest,
            &source_expected,
            source.as_fd(),
            &target_expected,
        ...
        reject_changed_fscrypt(&parsed.manifest, &target_objects, &target_paths)?;
        ...
        let changes = self
            .store
            .publish_historical_broker_comparison(&claim, &parsed.manifest, &events, now_ns)
    ...
    fn finish_cut(&mut self, completion: CutCompletion) -> Result<PublishedCut, ServiceError> {
        ...
        let (_live_path, _indexed_base_path) = self.watch_paths(reservation.watch_id)?;
        ...
        let mut incremental_phase = "load-or-compare-manifest";
        let mut failure_spool_path = spool_path.clone();
        ...
        let incremental = (|| -> Result<PublishedCut, ServiceError> {
            ...
            let (parsed, comparison_result) = if let Some(staged) = staged_manifest {
                ...
                let mut spool = create_private_spool(&spool_path)?;
                incremental_phase = "compare-immutable-snapshots";
                ...
                incremental_phase = "read-changed-object-manifest";
                ...
                incremental_phase = "parse-changed-object-manifest";
            ...
            incremental_phase = "validate-changed-object-proof";
            ...
            if parsed.target_objects.is_none() {
                incremental_phase = "scan-legacy-nested-subvolumes";
            ...
            incremental_phase = "validate-dirty-witness";
            require_dirty_witness_contract(parsed.dirty_witness_contract)?;
            let required: BTreeSet<_> = parsed
                .manifest
                .objects
            ...
            incremental_phase = "resolve-target-objects";
            ...
            incremental_phase = "resolve-endpoint-paths";
            let resolve_paths_started = std::time::Instant::now();
            let (base_paths, target_paths) = self.resolve_changed_endpoint_paths(
                &parsed.manifest,
                &parent_expected,
                base.as_fd(),
            ...
            tracing::info!(
                elapsed_ms = resolve_paths_started.elapsed().as_millis() as u64,
                changed_inodes = parsed.manifest.objects.len(),
                "query cut broker endpoint paths resolved"
            );
            reject_changed_fscrypt(&parsed.manifest, &target_objects, &target_paths)?;
            let replay_events = derive_events_from_endpoint_paths(
                &parsed.manifest,
                &target_objects,
                &base_paths,
                &target_paths,
            );
            incremental_phase = "append-replay-spool";
            let replay_payload_hash = append_replay_spool_payload(&spool_path, &replay_events)
                .map_err(|error| ServiceError::context("append replay spool", error))?;
            let replay_path = self.config.spool_directory.join(format!(
                "replay-{}-{}.spool",
                hex_id(&reservation.watch_id),
                reservation.sequence
            ));
            incremental_phase = "finalize-replay-spool";
            fs::rename(&spool_path, &replay_path)
                .map_err(|error| ServiceError::context("finalize replay spool", error))?;
            failure_spool_path = replay_path.clone();
            if !physical_published {
                ...
                self.store
                    ...
                    .map_err(|error| {
                        ...
                        ServiceError::context("publish physical cut", error)
                    })?;
                physical_published = true;
            }
            incremental_phase = "publish-indexed-cut";
            ...
            let published = self
                ...
                .publish_adjacent_broker_delta(
                    ...
                    &parsed.manifest,
                    &replay_events,
                    Some((replay_path.as_os_str().as_bytes(), replay_payload_hash)),
                    completion.now_ns,
                )
                .map_err(|error| {
                    let _ = fs::remove_file(&replay_path);
                    ServiceError::context("publish indexed cut", error)
                })?;
            tracing::info!(
        ...
        match incremental {
            ...
            Err(incremental_error) => {
                tracing::error!(
                    ...
                    base_snapshot_id = reservation.base_snapshot_id,
                    target_snapshot_id = recorded.snapshot_id,
                    target_snapshot_uuid = %Uuid::from_bytes(recorded.identity.subvol_uuid),
                    manifest_spool = %failure_spool_path.display(),
                    elapsed_ms = finish_cut_started.elapsed().as_millis() as u64,
                ...
                if let Err(failure) = failure {
                    tracing::error!(
                        ...
                        operation_id = %hex_id(&reservation.operation_id),
                        cut_sequence = reservation.sequence,
                        "failed to record rejected incremental cut"
                    );
                    return Err(ServiceError::new(format!(
                        "{incremental_error}; record rejected incremental cut: {failure}"
                    )));
                }
                Err(incremental_error)
            }
    ...
    /// Collects daemon-free scan state with the same consumer-aware replay
    /// retention policy as ordinary maintenance.
    ///
    /// Git and JJ are independent durable endpoints on one watch timeline.
    /// A direct caller may advance only its own pin; shared history remains
    /// retained from the oldest committed or pending consumer through the
    /// newest cut.
    pub fn garbage_collect_direct(
        ...
        self.garbage_collect(now_ns, limit)
    }
    ...
    pub fn maintain_history(&mut self, now_ns: i64) -> Result<usize, ServiceError> {
        ...
        for watch_id in watches {
            reclaimed += self
                ...
                .map_err(|error| ServiceError::context("retain shared replay history", error))?;
        }
        Ok(reclaimed)
    }
    ...
    fn execute_snapshot_delete(
        ...
        let entry_name =
            managed_snapshot_entry_name(&path, &self.config.managed_snapshot_directory)?;
        let open_verify_started = Instant::now();
            ...
            .map_err(|error| ServiceError::context("inspect snapshot GC target", error))?
        {
            ...
            log::debug!(
                "AWACS snapshot GC phase completed: phase=open and verify elapsed={:?}",
                open_verify_started.elapsed()
            );
            ...
            let result = self
                ...
                .map_err(|error| ServiceError::context("trash managed snapshot", error))?;
            log::debug!(
                "AWACS snapshot GC phase completed: phase=broker trash verify elapsed={:?}",
                Duration::from_nanos(result.verify_elapsed_ns)
            );
            log::debug!(
                "AWACS snapshot GC phase completed: phase=trash rename elapsed={:?} round_trip={:?}",
                Duration::from_nanos(result.rename_elapsed_ns),
                trash_started.elapsed()
            );
        } else {
            log::debug!(
                "AWACS snapshot GC phase completed: phase=open and verify elapsed={:?} target_present=false",
                open_verify_started.elapsed()
            );
        }
        if path
        ...
        {
            return Err(ServiceError::new(
                "snapshot still exists after broker trash rename",
            ));
        }
        ...
        log::debug!(
            "AWACS snapshot GC phase completed: phase=durable tombstone elapsed={:?}",
            tombstone_started.elapsed()
        );
        Ok(())
    }
    ...
    fn create_snapshot(
        &mut self,
        _write_access: &SnapshotLifecycleWriteAccess,
        ...
        let entry_path = self
            .config
            .managed_snapshot_directory
            .join(std::ffi::OsString::from_vec(entry_name.to_vec()));
        create_private_snapshot_entry(&entry_path)?;
        let destination = File::open(&entry_path)
            .map_err(|error| ServiceError::context("open managed snapshot entry", error))?;
        create_btrfs_snapshot(
            source.as_fd(),
            destination.as_fd(),
            MANAGED_SNAPSHOT_CHILD_NAME,
            readonly,
        )
        .map_err(|error| ServiceError::context("create snapshot", error))?;
        let path = entry_path.join(std::ffi::OsString::from_vec(
            MANAGED_SNAPSHOT_CHILD_NAME.to_vec(),
        ));
        let snapshot = self
            .open_subvolume(&path)
    ...
    /// Resolves every changed inode against one immutable endpoint through
    /// the broker's bounded batched reverse-lookup ioctl.
    fn resolve_endpoint_paths(
        ...
        inodes: &BTreeSet<u64>,
        endpoint_name: &str,
    ) -> Result<BTreeMap<u64, Vec<Vec<u8>>>, ServiceError> {
        if inodes.is_empty() {
            return Ok(BTreeMap::new());
        }
        let token = hex_id(&random_id());
        let input_path = self
            .config
            .spool_directory
            .join(format!("paths-{endpoint_name}-{token}.in"));
        let output_path = self
            .config
            .spool_directory
            .join(format!("paths-{endpoint_name}-{token}.out"));
        let result = (|| -> Result<BTreeMap<u64, Vec<Vec<u8>>>, ServiceError> {
            let mut input = create_private_spool(&input_path)?;
            for ino in inodes {
                input
                    .write_all(&ino.to_be_bytes())
                    .map_err(|error| ServiceError::context("write inode-path input", error))?;
            }
            let mut output = create_private_spool(&output_path)?;
            let inode_count = u64::try_from(inodes.len())
                .map_err(|_| ServiceError::new("inode-path input count exceeds u64"))?;
            let lookup = self
                .broker
                .inode_paths(
                    &InodePathsExecution {
                        ...
                        inode_count,
                        max_output_bytes: MAX_INODE_PATH_OUTPUT,
                    },
                    snapshot,
                    input.as_fd(),
                    output.as_fd(),
                ...
                .map_err(|error| {
                    ServiceError::context(
                        format!("resolve {endpoint_name} changed inode paths"),
                        error,
                    )
                })?;
            output
                .seek(SeekFrom::Start(0))
                .map_err(|error| ServiceError::context("rewind inode-path output", error))?;
            let mut bytes = Vec::with_capacity(
                usize::try_from(lookup.output_bytes)
                    .map_err(|_| ServiceError::new("inode-path output exceeds usize"))?,
            );
            (&mut output)
                .take(lookup.output_bytes + 1)
                .read_to_end(&mut bytes)
                .map_err(|error| ServiceError::context("read inode-path output", error))?;
            if bytes.len() as u64 != lookup.output_bytes {
                return Err(ServiceError::new(
                    "inode-path output differs from broker length",
                ));
            }
            parse_inode_path_output(&bytes, inode_count)
        })();
    ...
    fn resolve_changed_endpoint_paths(
        &self,
        manifest: &ChangedObjectsManifest,
        parent_expected: &ExpectedSubvolume,
        parent: BorrowedFd<'_>,
        target_expected: &ExpectedSubvolume,
        target: BorrowedFd<'_>,
    ) -> Result<(BTreeMap<u64, Vec<Vec<u8>>>, BTreeMap<u64, Vec<Vec<u8>>>), ServiceError> {
        let inodes: BTreeSet<_> = manifest.objects.keys().copied().collect();
        let base_paths = self.resolve_endpoint_paths(parent_expected, parent, &inodes, "base")?;
        let target_paths =
            self.resolve_endpoint_paths(target_expected, target, &inodes, "target")?;
        Ok((base_paths, target_paths))
    }
...
fn probe_writable_directory(path: &Path, context: &str) -> Result<(), ServiceError> {
    let directory = File::open(path).map_err(|error| ServiceError::context(context, error))?;
    probe_writable_directory_fd(directory.as_fd())
        .map_err(|error| ServiceError::context(context, error))
}

/// Proves write access without changing the visible namespace when the
/// filesystem supports O_TMPFILE. The named fallback is only for older
/// filesystems and is removed before this returns.
fn probe_writable_directory_fd(directory: BorrowedFd<'_>) -> io::Result<()> {
    let dot = CString::new(".").expect("static directory name has no NUL");
    // SAFETY: directory remains live, dot is NUL terminated, and a successful
    // descriptor is immediately adopted into OwnedFd.
    let opened = unsafe {
        libc::openat(
            directory.as_raw_fd(),
            dot.as_ptr(),
    ...
    if opened >= 0 {
        ...
        let _probe = unsafe { OwnedFd::from_raw_fd(opened) };
        return Ok(());
    }
    ...
    let name = CString::new(format!(".awacs-write-probe-{}", Uuid::new_v4()))
        .expect("UUID-backed probe name has no NUL");
    // SAFETY: directory remains live, name is NUL terminated, and
    // O_CREAT|O_EXCL keeps the probe from replacing an existing entry.
    let opened = unsafe {
        libc::openat(
            directory.as_raw_fd(),
            name.as_ptr(),
    ...
    // SAFETY: openat returned one new owned descriptor.
    let _probe = unsafe { OwnedFd::from_raw_fd(opened) };
    // SAFETY: directory remains live and name is NUL terminated.
...
fn reject_managed_descendant(
    ...
    if !managed.starts_with(&source_path) {
        return Ok(());
    }

    // Root snapshots omit nested Btrfs subvolumes. A colocated main worktree
    // keeps `.git` as one such boundary, so `.git/awacs` is not part of the
    // immutable snapshot namespace even though it is lexically below the
    // watched root. Require a same-filesystem, different-root-id boundary;
    // an ordinary in-tree directory must still be rejected before snapshot
    // creation can recurse into manager state.
    let relative = managed
        .strip_prefix(&source_path)
        .expect("starts_with proved managed is below source");
    let mut ancestor = source_path.to_path_buf();
    for component in relative.components() {
        ancestor.push(component);
        let metadata = fs::metadata(&ancestor).map_err(|error| {
            ServiceError::context(
                format!("stat managed snapshot ancestor {}", ancestor.display()),
                error,
            )
        })?;
        ...
        let boundary = OpenedSubvolume::open(&ancestor).map_err(|error| {
            ServiceError::context(
                format!("inspect managed snapshot ancestor {}", ancestor.display()),
                error,
            )
        })?;
        if boundary.filesystem.fs_uuid == source.filesystem.fs_uuid
            && boundary.subvolume.root_id != source.subvolume.root_id
        {
            return Ok(());
        }
    }
    Err(ServiceError::new(
        "managed snapshot directory must not be inside the watched source unless it is below a distinct nested Btrfs subvolume",
    ))
}
...
fn is_terminal_unpublished_cut_rejection(error: &ServiceError) -> bool {
    ...
    [
        ...
        "changed-objects detected an fscrypt introduction",
...
fn parse_inode_path_output(
    bytes: &[u8],
    expected_inode_count: u64,
) -> Result<BTreeMap<u64, Vec<Vec<u8>>>, ServiceError> {
    ...
    if bytes.len() < header_len || bytes.get(..4) != Some(INODE_PATH_OUTPUT_MAGIC.as_slice()) {
        return Err(ServiceError::new("inode-path output has invalid header"));
    }
    if take_output_u16(bytes, 4)? != INODE_PATH_OUTPUT_VERSION
        || usize::from(take_output_u16(bytes, 6)?) != header_len
    {
        return Err(ServiceError::new(
            "inode-path output has unsupported version or header length",
        ));
    }
    let inode_count = take_output_u64(bytes, 8)?;
    if inode_count != expected_inode_count {
        return Err(ServiceError::new(
            "inode-path output count differs from requested input",
        ));
    }
    let expected_paths = take_output_u64(bytes, 16)?;
    let mut cursor = header_len;
    let mut paths = BTreeMap::new();
    let mut actual_paths = 0_u64;
    for _ in 0..inode_count {
        let ino = take_output_u64(bytes, cursor)?;
        cursor = cursor
            .checked_add(8)
            .ok_or_else(|| ServiceError::new("inode-path cursor overflow"))?;
        let count = usize::try_from(take_output_u32(bytes, cursor)?)
            .map_err(|_| ServiceError::new("inode-path record count exceeds usize"))?;
        cursor = cursor
            .checked_add(4)
            .ok_or_else(|| ServiceError::new("inode-path cursor overflow"))?;
        let mut inode_paths = Vec::with_capacity(count);
        for _ in 0..count {
            let length = usize::try_from(take_output_u32(bytes, cursor)?)
                .map_err(|_| ServiceError::new("inode-path length exceeds usize"))?;
            cursor = cursor
                .checked_add(4)
                .ok_or_else(|| ServiceError::new("inode-path cursor overflow"))?;
            let end = cursor
                .checked_add(length)
                .ok_or_else(|| ServiceError::new("inode-path range overflow"))?;
            let path = bytes
                .get(cursor..end)
                .ok_or_else(|| ServiceError::new("inode-path output is truncated"))?
                .to_vec();
            cursor = end;
            inode_paths.push(path);
            actual_paths = actual_paths
                .checked_add(1)
                .ok_or_else(|| ServiceError::new("inode-path result count overflow"))?;
        }
        if ino == 0 || paths.insert(ino, inode_paths).is_some() {
            return Err(ServiceError::new(
                "inode-path output contains an invalid or duplicate inode",
            ));
        }
    }
    if cursor != bytes.len() || actual_paths != expected_paths {
        return Err(ServiceError::new(
            "inode-path output has trailing bytes or mismatched path count",
        ));
    }
    Ok(paths)
}

/// Changed-objects v2 carries the exact target security xattrs for changed
/// inodes. Reject the transition before publication instead of keeping a
/// whole-tree userspace index solely to rediscover fscrypt later.
fn reject_changed_fscrypt(
    manifest: &ChangedObjectsManifest,
    target_objects: &BTreeMap<u64, Object>,
    target_paths: &BTreeMap<u64, Vec<Vec<u8>>>,
) -> Result<(), ServiceError> {
    for change in manifest.objects.values() {
        if change.is_deleted() || change.change_mask & (CHANGE_CREATED | CHANGE_XATTR) == 0 {
            continue;
        }
        let Some(object) = target_objects.get(&change.ino) else {
            continue;
        };
        ...
        let path = target_paths
            ...
            .unwrap_or_else(|| "<no resolved path>".to_owned());
        return Err(ServiceError::new(format!(
            "changed-objects detected an fscrypt introduction at inode {} path {}; fscrypt snapshots are unsupported",
            change.ino, path
        )));
    }
    Ok(())
...
fn take_output_u16(bytes: &[u8], offset: usize) -> Result<u16, ServiceError> {
    let value = bytes
        .get(offset..offset.saturating_add(2))
        .ok_or_else(|| ServiceError::new("inode-path output is truncated"))?;
    Ok(u16::from_be_bytes(
        value.try_into().expect("fixed u16 slice"),
    ))
}

fn take_output_u32(bytes: &[u8], offset: usize) -> Result<u32, ServiceError> {
    let value = bytes
        .get(offset..offset.saturating_add(4))
        .ok_or_else(|| ServiceError::new("inode-path output is truncated"))?;
    Ok(u32::from_be_bytes(
        value.try_into().expect("fixed u32 slice"),
    ))
}

fn take_output_u64(bytes: &[u8], offset: usize) -> Result<u64, ServiceError> {
    let value = bytes
...
fn quarantine_unexpected_managed_entries(
    ...
    let mut known = BTreeSet::<Vec<u8>>::new();
    {
        ...
        for (path, state) in snapshots {
            ...
            let entry_path = managed_snapshot_entry_path(&path_buf, managed_directory)?;
            ...
            if !matches!(state.as_str(), "deleted" | "lost") {
                known.insert(entry_path.as_os_str().as_bytes().to_vec());
            }
        }
    }
    {
        ...
        for path in paths {
            ...
            let entry_path = managed_snapshot_entry_path(&path, managed_directory)?;
            known.insert(entry_path.as_os_str().as_bytes().to_vec());
        }
    }
...
fn managed_snapshot_path(managed_directory: &Path, entry_name: &[u8]) -> PathBuf {
    managed_directory
        .join(OsString::from_vec(entry_name.to_vec()))
        .join(OsString::from_vec(MANAGED_SNAPSHOT_CHILD_NAME.to_vec()))
}

fn managed_snapshot_entry_path(
    snapshot_path: &Path,
    managed_directory: &Path,
) -> Result<PathBuf, ServiceError> {
    if snapshot_path.file_name().map(|name| name.as_bytes()) != Some(MANAGED_SNAPSHOT_CHILD_NAME) {
        return Err(ServiceError::new(
            "stored managed snapshot path has an unexpected child name",
        ));
    }
    let entry_path = snapshot_path
        .parent()
        .ok_or_else(|| ServiceError::new("stored managed snapshot path has no entry directory"))?;
    if entry_path.parent() != Some(managed_directory) {
        return Err(ServiceError::new(
    ...
    let entry_name = entry_path
        .file_name()
        .ok_or_else(|| ServiceError::new("stored managed snapshot entry has no basename"))?;
    if !is_managed_object_name(entry_name.as_bytes()) {
        return Err(ServiceError::new(
            "stored managed snapshot entry has an invalid basename",
        ));
    }
    Ok(entry_path.to_path_buf())
}

fn managed_snapshot_entry_name(
    snapshot_path: &Path,
    managed_directory: &Path,
) -> Result<Vec<u8>, ServiceError> {
    let entry_path = managed_snapshot_entry_path(snapshot_path, managed_directory)?;
    Ok(entry_path
        .file_name()
        .expect("validated managed snapshot entry has a basename")
        .as_bytes()
        .to_vec())
}

fn create_private_snapshot_entry(path: &Path) -> Result<(), ServiceError> {
...
mod tests {
    ...
    #[test]
    fn changed_objects_fscrypt_introduction_is_explicitly_rejected() {
        let manifest = ChangedObjectsManifest {
            objects: [(
                300,
                crate::manifest::ObjectChange {
                    ino: 300,
                    old_generation: None,
                    new_generation: Some(301),
                    change_mask: CHANGE_CREATED | CHANGE_XATTR,
                },
            )]
            .into(),
        ...
        let target_objects = BTreeMap::from([(
            300,
            Object {
                ino: 300,
                generation: 301,
        ...
        let paths = BTreeMap::from([(300, vec![b"encrypted".to_vec()])]);
        let error = reject_changed_fscrypt(&manifest, &target_objects, &paths).unwrap_err();
        assert!(error.to_string().contains("fscrypt introduction"));
    ...
    fn v2_proof_and_invalid_target_failures_are_terminal_before_publication() {
        for message in [
            ...
            r#"changed-objects detected an fscrypt introduction at inode 300 path "encrypted""#,
        ] {
            assert!(
    ...
    #[test]
    fn managed_snapshot_paths_use_private_wrapper_entries() {
        let managed = Path::new("/state/managed");
        let entry_name = b"cut-0123456789abcdef0123456789abcdef";
        let snapshot_path = managed_snapshot_path(managed, entry_name);
        assert_eq!(
            snapshot_path,
            PathBuf::from("/state/managed/cut-0123456789abcdef0123456789abcdef/snapshot")
        );
        assert_eq!(
            managed_snapshot_entry_name(&snapshot_path, managed).unwrap(),
            entry_name
        );
    }

    #[test]
    fn managed_snapshot_paths_reject_direct_entries() {
        let managed = Path::new("/state/managed");
        let direct = managed.join("cut-0123456789abcdef0123456789abcdef");
        assert!(
            managed_snapshot_entry_name(&direct, managed)
                .unwrap_err()
                .to_string()
                .contains("unexpected child name")
        );
    }
