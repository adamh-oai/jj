...
impl FacadeService {
    ...
    pub fn activate(
        ...
        // Every published revision is already proved fscrypt-free: snapshot-index
        // publication rejects it, and incremental publication rejects target
        // objects before advancing the revision. Snapshot worktrees clone
        // only an already-accepted index into their own initial revision, so
        // rehydrating the complete index here only to repeat the fscrypt scan
        // is both redundant and O(all paths).
    ...
    fn prepare_scan_after_cut(
        ...
        let projection = if let Some(from_snapshot_uuid) = previous_snapshot_uuid {
            ...
            // snapshots and old path-map revisions while retained cursors
            // still receive a conservative union of every intervening cut.
            match self.service.store_mut().replay_historical_changes(
                ...
                Err(error) => {
                    tracing::warn!(
                        ?error,
                        "direct retained-event replay could not prove a delta"
                    );
                    Projection {
                        fresh_instance: true,
                        paths: vec![b"/".to_vec()],
    ...
    /// Releases old facade-boundary references after a daemon-free caller has
    /// committed to event-journal replay. This does not release active query
    /// leases and therefore cannot invalidate an already-open scan response.
    pub fn release_historical_boundary_snapshots(
        ...
        self.service
            .store_mut()
            .release_historical_boundary_snapshots(
                prepared.activation.watch_id,
                prepared.result.sequence,
            )
            .map(|_| ())
            .map_err(|error| FacadeError::context("release historical snapshot boundaries", error))
    }
