...
use crate::broker::{
    ChangedObjectsExecution, ChangedObjectsResult, ExpectedSubvolume, Frame, InodePathsExecution,
    InodePathsResult, NestedSubvolumesExecution, Opcode, PeerCredentials, SeqPacket, SessionGate,
    SnapshotTrash, SnapshotTrashExecution, SnapshotTrashResult, execute_changed_objects,
    execute_has_nested_subvolumes, execute_inode_paths, execute_snapshot_trash,
    execute_snapshot_trash_drain,
};
...
pub struct BrokerDispatcher {
    manager_uid: u32,
    gate: SessionGate,
    snapshot_trash: SnapshotTrash,
}

impl BrokerDispatcher {
    pub fn new(manager_uid: u32) -> Result<Self, BrokerError> {
        Ok(Self {
            manager_uid,
            gate: SessionGate::default(),
            snapshot_trash: SnapshotTrash::new()?,
        })
    }
    ...
    fn dispatch(
        ...
        match frame.opcode {
            ...
            Opcode::InodePaths => {
                require_fd_count(fds, 3)?;
                let request = InodePathsExecution {
                    target: decoder.expected_subvolume()?,
                    owner_uid: decoder.u32()?,
                    inode_count: decoder.u64()?,
                    max_output_bytes: decoder.u64()?,
                };
                decoder.finish()?;
                let result =
                    execute_inode_paths(&request, fds[0].as_fd(), fds[1].as_fd(), fds[2].as_fd())?;
                Ok(encode_inode_paths_result(&result))
            }
            Opcode::TrashSnapshot => {
                require_fd_count(fds, 3)?;
                let request = SnapshotTrashExecution {
                    target: decoder.expected_subvolume()?,
                    entry_name: decoder.bytes()?.to_vec(),
                    trash_name: decoder.bytes()?.to_vec(),
                };
                decoder.finish()?;
                let result = execute_snapshot_trash(
                    &self.snapshot_trash,
                    &request,
                    self.manager_uid,
                    fds[0].as_fd(),
                ...
                Ok(encode_snapshot_trash_result(&result))
            }
            Opcode::DrainSnapshotTrash => {
                require_fd_count(fds, 1)?;
                let trash_name = decoder.bytes()?.to_vec();
                decoder.finish()?;
                execute_snapshot_trash_drain(
                    &self.snapshot_trash,
                    self.manager_uid,
...
impl BrokerClient {
    ...
    pub fn inode_paths(
        ...
        let mut encoder = self.auth_encoder();
        encoder.expected_subvolume(&request.target);
        encoder.u32(request.owner_uid);
        encoder.u64(request.inode_count);
        encoder.u64(request.max_output_bytes);
        self.socket.send(
            &Frame::new(Opcode::InodePaths, encoder.finish())?,
            &[target, input, output],
        )?;
        let response = receive_response(&self.socket, Opcode::InodePaths)?;
        decode_inode_paths_result(&response)
    }

    pub fn trash_snapshot(
        &self,
        request: &SnapshotTrashExecution,
        source_parent: BorrowedFd<'_>,
        trash_parent: BorrowedFd<'_>,
        target: BorrowedFd<'_>,
    ) -> Result<SnapshotTrashResult, BrokerError> {
        let mut encoder = self.auth_encoder();
        encoder.expected_subvolume(&request.target);
        encoder.bytes(&request.entry_name)?;
        encoder.bytes(&request.trash_name)?;
        self.socket.send(
            &Frame::new(Opcode::TrashSnapshot, encoder.finish())?,
            &[source_parent, trash_parent, target],
        )?;
        let response = receive_response(&self.socket, Opcode::TrashSnapshot)?;
        decode_snapshot_trash_result(&response)
    }

    pub fn drain_snapshot_trash(
        &self,
        trash_name: &[u8],
        trash_parent: BorrowedFd<'_>,
    ) -> Result<(), BrokerError> {
        let mut encoder = self.auth_encoder();
        encoder.bytes(trash_name)?;
        self.socket.send(
            &Frame::new(Opcode::DrainSnapshotTrash, encoder.finish())?,
            &[trash_parent],
        )?;
        let response = receive_response(&self.socket, Opcode::DrainSnapshotTrash)?;
        if !response.is_empty() {
            return Err(BrokerError::new(
                "broker returned an invalid snapshot trash drain result",
            ));
        }
        Ok(())
    }
...
fn encode_inode_paths_result(result: &InodePathsResult) -> Vec<u8> {
    let mut encoder = Encoder::default();
    encoder.u64(result.inode_count);
    encoder.u64(result.path_count);
    encoder.u64(result.output_bytes);
    encoder.finish()
}

fn decode_inode_paths_result(bytes: &[u8]) -> Result<InodePathsResult, BrokerError> {
    let mut decoder = Decoder::new(bytes);
    let result = InodePathsResult {
        inode_count: decoder.u64()?,
        path_count: decoder.u64()?,
        output_bytes: decoder.u64()?,
    };
    decoder.finish()?;
    Ok(result)
...
fn encode_snapshot_trash_result(result: &SnapshotTrashResult) -> Vec<u8> {
    let mut encoder = Encoder::default();
    encoder.u64(result.verify_elapsed_ns);
    encoder.u64(result.rename_elapsed_ns);
    encoder.finish()
}

fn decode_snapshot_trash_result(bytes: &[u8]) -> Result<SnapshotTrashResult, BrokerError> {
    let mut decoder = Decoder::new(bytes);
    let result = SnapshotTrashResult {
        verify_elapsed_ns: decoder.u64()?,
        rename_elapsed_ns: decoder.u64()?,
    };
    decoder.finish()?;
    Ok(result)
}
...
impl Encoder {
    ...
    fn bytes(&mut self, value: &[u8]) -> Result<(), BrokerError> {
        let length = u32::try_from(value.len())
            .map_err(|_| BrokerError::new("broker byte string exceeds u32 length"))?;
        self.u32(length);
        self.bytes.extend_from_slice(value);
        Ok(())
    }
...
impl<'a> Decoder<'a> {
    ...
    fn bytes(&mut self) -> Result<&'a [u8], BrokerError> {
        let length = usize::try_from(self.u32()?)
            .map_err(|_| BrokerError::new("broker byte string length exceeds usize"))?;
        self.take(length)
    }
...
mod tests {
    ...
    fn handshake_authenticates_peer_and_fences_old_session() {
        ...
        let server = thread::spawn(move || {
            let dispatcher = BrokerDispatcher::new(uid).unwrap();
    ...
    #[test]
    fn snapshot_trash_result_wire_preserves_phase_timings() {
        let result = SnapshotTrashResult {
            verify_elapsed_ns: 17,
            rename_elapsed_ns: 23,
        };
        assert_eq!(
            decode_snapshot_trash_result(&encode_snapshot_trash_result(&result)).unwrap(),
            result
        );
    }

    #[test]
    fn inode_paths_result_wire_preserves_counts() {
        let result = InodePathsResult {
            inode_count: 100_000,
            path_count: 100_001,
            output_bytes: 2_400_123,
        };
        assert_eq!(
            decode_inode_paths_result(&encode_inode_paths_result(&result)).unwrap(),
            result
        );
    }
}
