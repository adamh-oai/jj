...
use crate::index::{
    Event, EventKind, Index, Object, ROOT_INO, apply_manifest,
    apply_manifest_to_trusted_checkpoint, object_security_digest, object_state_digest,
    reference_state_digest, xor_digest,
};
use crate::manifest::{
    CHANGE_CREATED, CHANGE_INODE, CHANGE_REF, ChangedObjectsManifest, Reference,
};
...
impl Store {
    ...
    /// Drops durable facade-boundary references older than one committed cut.
    ///
    /// Daemon-free direct scans replay the adjacent change-event journal and
    /// therefore do not need old physical snapshots merely because an older
    /// cursor may be presented later. Active query leases still pin any
    /// snapshot whose fd is currently being scanned.
    pub fn release_historical_boundary_snapshots(
        &mut self,
        watch_id: [u8; 16],
        keep_sequence: i64,
    ) -> Result<usize, ManagerError> {
        if keep_sequence < 0 {
            return Err(ManagerError::new(
                "historical boundary keep sequence must be non-negative",
            ));
        }
        self.connection_mut()
            .execute(
                "DELETE FROM fsmonitor_boundaries
                 WHERE watch_id = ?1 AND cut_sequence < ?2",
                params![watch_id.as_slice(), keep_sequence],
            )
            .map_err(ManagerError::from)
    }
...
/// Returns one repository-relative path for an fscrypt directory in a fully
/// applied endpoint index. Checking the endpoint, rather than raw records,
/// allows a transient create/remove to net out before publication.
fn first_fscrypt_path(index: &Index) -> Result<Option<Vec<u8>>, ManagerError> {
    let Some(object) = index
        .objects
        .values()
    ...
    index
        .paths(object.ino)
        .map_err(|error| ManagerError::new(format!("resolve fscrypt directory path: {error}")))?
        .into_iter()
        .next()
        .map(Some)
        .ok_or_else(|| ManagerError::new("fscrypt directory has no reachable path"))
}
...
mod tests {
    ...
    use crate::manifest::{
        CHANGE_CREATED, CHANGE_DELETED, CHANGE_FILE_DATA, CHANGE_INODE, CHANGE_REF, ObjectChange,
    };
    use crate::namespace::ViewBinding;
    ...
    #[test]
    fn fscrypt_check_uses_only_the_applied_endpoint() {
        let mut target = Index::default();
        target
            .objects
            .insert(ROOT_INO, object(ROOT_INO, MODE_DIRECTORY | 0o755, 1));
        let mut encrypted = object(300, MODE_DIRECTORY | 0o700, 1);
        encrypted.privilege_flags = PRIVILEGE_FSCRYPT;
        target.objects.insert(300, encrypted);
        target.references.insert(Reference {
            ino: 300,
            parent_ino: ROOT_INO,
            name: b"encrypted".to_vec(),
        });
        assert_eq!(
            first_fscrypt_path(&target).unwrap(),
    ...
    fn finds_a_snapshot_descendant_seed_with_shared_immutable_history() {
        ...
        let seed_revision_id = store
            ...
            .unwrap()
            .unwrap();
        let seed_index = store.load_revision(seed_revision_id).unwrap();
        let child_metadata = ServiceMetadata {
            store_uuid: [20; 16],
            clock_hmac_key: [21; 32],
            clock_format_version: 1,
            last_boot_id: [22; 16],
            created_ns: 1,
        };
        ...
        child_request.reserved_snapshot_path = b"/store/snapshots/w/s-child".to_vec();
        let child_reservation = child_store.reserve_initialize(&child_request).unwrap();
        child_store
            .start_initialize_filesystem_effect(&child_reservation, child_request.lease_owner, 400)
            .unwrap();
        let child_identity = SnapshotIdentity {
            fs_uuid: child_request.fs_uuid,
            subvol_uuid: [43; 16],
            parent_uuid: Some(child_request.source_subvol_uuid),
            received_uuid: None,
            root_id: 901,
            ctransid: 11,
            otransid: 10,
            path: child_request.reserved_snapshot_path.clone(),
            readonly: true,
            created_ns: 401,
        };
        let child_snapshot = child_store
        ...
        let child = child_store
            ...
            .unwrap();
        assert_ne!(child_store.path(), store.path());
        let child_watch_count: i64 = child_store
            .connection()
            .query_row("SELECT count(*) FROM watches", [], |row| row.get(0))
            .unwrap();
        assert_eq!(child_watch_count, 1);
        assert_eq!(
        ...
        assert_eq!(
            store
                .descendant_seed_revision(request.fs_uuid, [99; 16])
                .unwrap(),
            None
        );
    }
    ...
    #[test]
    fn direct_boundary_release_keeps_logical_cut_history_replayable() {
        let (_temp, mut store, request) = setup();
        let (_initialize, initialized) = initialize_watch(&mut store, &request);
        for sequence in 1..=3 {
            append_ready_boundary(&mut store, &initialized, sequence);
        ...
        assert_eq!(
            store
                .release_historical_boundary_snapshots(initialized.watch_id, 3)
                .unwrap(),
            2
        );
        let boundaries: Vec<i64> = store
            .connection()
            .prepare(
                "SELECT cut_sequence FROM fsmonitor_boundaries \
                 WHERE watch_id = ?1 ORDER BY cut_sequence",
            )
            .unwrap()
            .query_map([initialized.watch_id.as_slice()], |row| row.get(0))
            .unwrap()
        ...
        let (replay_floor, cuts): (i64, i64) = store
            .connection()
            .query_row(
                "SELECT w.replay_floor_seq, count(c.sequence) \
                   FROM watches w JOIN watch_cuts c ON c.watch_id = w.id \
        ...
        // Physical-boundary rows are not the cursor-to-sequence index. The
        // tombstoned snapshot identity rows and logical cuts still resolve an
        // older token; these full-fresh fixture cuts conservatively request a
        // full scan rather than failing as an unknown cursor.
        let replay = store
            ...
            .unwrap();
        assert_eq!((replay.from_sequence, replay.to_sequence), (1, 3));
        assert!(replay.fresh_instance);
        assert!(store.foreign_key_violations().unwrap().is_empty());
    }
    ...
    fn consumer_baseline_handoff_keeps_only_committed_and_pending() {
        ...
        store.release_consumer_baseline(owner).unwrap();
        // A lost response may cause the consumer to retry the release.
        store.release_consumer_baseline(owner).unwrap();
        let count: i64 = store
            .connection()
            .query_row(
                "SELECT count(*) FROM snapshot_pins WHERE owner_kind = 'consumer-baseline' AND owner_id = ?1",
                [owner.as_slice()],
                |row| row.get(0),
            )
            .unwrap();
    ...
    #[test]
    fn direct_query_fence_survives_a_newer_short_lived_activation() {
        let (_temp, mut store, request) = setup();
        let (_reservation, initialized) = initialize_watch(&mut store, &request);
        append_ready_boundary(&mut store, &initialized, 1);
        let binding = |monitor_session_id| ViewBinding {
            monitor_session_id,
            root_path: request.source_path.clone(),
            fs_uuid: request.fs_uuid,
            subvol_uuid: request.source_subvol_uuid,
        ...
        store
            .connection_mut()
            .execute(
                "UPDATE fsmonitor_boundaries SET clock_epoch = ?2 \
                 WHERE watch_id = ?1 AND cut_sequence = 1",
                params![
                    initialized.watch_id.as_slice(),
                    first.clock_epoch.as_slice()
                ],
            )
            .unwrap();
        let lease = store
            .begin_query_lease(&first, None, 1, [32; 16], 1_000)
            .unwrap();
        let _second = store
            .activate_snapshot_facade(
                initialized.watch_id,
        ...
        assert!(store.renew_query_lease(&lease, &first, 100, 2_000).is_err());
        store.renew_query_lease_direct(&lease, 100, 2_000).unwrap();
        store.release_query_lease_direct(&lease).unwrap();
        let state: String = store
            .connection()
            .query_row(
                "SELECT state FROM query_leases WHERE id = ?1",
