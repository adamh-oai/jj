...
fn run_init(root: &Path, compress: Option<bool>, keep: bool) -> Result<(), String> {
    ...
    let initialized = initialize_root(root, |progress| {
        ...
        match progress {
            InitProgress::Phase(phase) => eprintln!("AWACS init: {phase}..."),
            InitProgress::Index(counts) => {
                eprint!(
                    "\rAWACS init: indexing initial snapshot: {} directories, {} objects, {} paths",
...
fn run_git_fsmonitor_hook() -> Result<(), String> {
    ...
    let previous_baseline = decode_git_fsmonitor_token(prior_token.as_bytes());
    let mut client = DirectScanClient::for_root(&root)
        .map_err(|error| format!("open AWACS fsmonitor root: {error}"))?;
    let mut lease = client
        .begin_scan(&BeginScanRequest {
            live_root: root,
            // Daemon-free Git replay has no durable baseline pin. The token
            // itself identifies the prior logical cut; this value is ignored
            // by the direct coordinator.
            baseline_owner_id: [0; 16],
            previous_baseline,
        })
    ...
    let paths = match &lease.invalidation {
        ...
        // Git adapter proves directory-prefix semantics, widen prefixes to
        // the documented full-invalidation sentinel.
        Invalidation::Full | Invalidation::Prefixes(_) => vec![b"/".to_vec()],
    };
    lease
        .promote()
...
fn run_broker_server(
    ...
    let dispatcher = std::sync::Arc::new(BrokerDispatcher::new(manager_uid));
    loop {
