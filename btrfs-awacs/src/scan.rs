...
pub trait ScanClient: Send {
    ...
    /// Releases every committed or pending baseline owned by one consumer.
    ///
    /// Consumers call this when they permanently stop using snapshot-backed
    /// state. The stable owner ID is an opaque capability, not a filesystem
    /// path or mutable commit identifier.
...
const OP_RELEASE_BASELINE: u16 = 5;
...
fn discover_scan_socket(live_root: &Path) -> Result<PathBuf, ScanError> {
    let command = std::env::var_os("BTRFS_AWACS_COMMAND")
        .unwrap_or_else(|| std::ffi::OsString::from("awacs"));
...
impl ScanClient for SocketScanClient {
    ...
    fn release_baseline(&mut self, baseline_owner_id: [u8; 16]) -> Result<(), ScanError> {
        let mut payload = Encoder::default();
        payload.bytes.extend_from_slice(&baseline_owner_id);
        let response = request_response(&self.socket, OP_RELEASE_BASELINE, payload.finish())?;
        let mut decoder = Decoder::new(&response.payload);
        decode_status(&mut decoder)?;
        if !response.fds.is_empty() {
            return Err(ScanError::new(
                ScanErrorKind::MalformedResponse,
                "AWACS ReleaseBaseline response carried descriptors",
            ));
        }
        decoder.finish()
    }
    ...
    pub fn serve_one(&self, socket: &ScanSocket) -> Result<(), ScanError> {
        ...
        match request.opcode {
            OP_BEGIN => {
                ...
                match handler.begin_scan(BeginScanRequest {
                    ...
                    Ok(lease) => {
                        ...
                        if result.is_err() {
                            // A failed response means the requesting socket
                            // disappeared before the client could own this
                            // lease. Abort the private session immediately
                            // instead of waiting for its TTL backstop.
                            let _ = handler.finish_scan(&session_id, ScanOutcome::Aborted);
                        }
            ...
            OP_RELEASE_BASELINE => {
                ...
                let baseline_owner_id = decoder.array()?;
                decoder.finish()?;
                match handler.release_baseline(baseline_owner_id) {
                    Ok(()) => socket.send(OP_RELEASE_BASELINE, FLAG_RESPONSE, &[0], &[]),
                    Err(err) => send_error(socket, OP_RELEASE_BASELINE, &err),
                }
            }
            _ => Err(ScanError::new(
                ScanErrorKind::MalformedResponse,
...
mod tests {
    ...
    fn socket_client_owns_private_framing_and_fd_lifecycle() {
        ...
        let server = std::thread::spawn(move || {
            ...
            let release = server_socket.receive().unwrap();
            assert_eq!(release.opcode, OP_RELEASE_BASELINE);
            let mut request = Decoder::new(&release.payload);
            assert_eq!(request.array::<16>().unwrap(), [9; 16]);
            request.finish().unwrap();
            server_socket
                .send(OP_RELEASE_BASELINE, FLAG_RESPONSE, &[0], &[])
                .unwrap();
        });
        ...
        lease.finish(ScanOutcome::Committed).unwrap();
        client.release_baseline([9; 16]).unwrap();
        server.join().unwrap();
    }
    ...
    fn dispatcher_serves_client_lifecycle() {
        ...
        impl ScanRequestHandler for Handler {
            ...
            fn release_baseline(&mut self, baseline_owner_id: [u8; 16]) -> Result<(), ScanError> {
                assert_eq!(baseline_owner_id, [9; 16]);
                Ok(())
            }
        }
    ...
    #[test]
    fn dispatcher_aborts_begin_session_when_response_socket_closes() {
        struct Handler {
            scan_root: PathBuf,
            outcomes: Arc<Mutex<Vec<ScanOutcome>>>,
        }
        ...
        impl ScanRequestHandler for Handler {
            ...
            fn renew_scan(&mut self, _session_id: &[u8]) -> Result<(), ScanError> {
                unreachable!()
            }

            fn promote_scan(&mut self, _session_id: &[u8]) -> Result<(), ScanError> {
                unreachable!()
            }
        ...
        request.bytes(b"/tmp/live").unwrap();
        request.bytes.extend_from_slice(&[9; 16]);
        request.optional_baseline(None).unwrap();
        client_socket
            .send(OP_BEGIN, 0, &request.finish(), &[])
            .unwrap();
        drop(client_socket);

        assert!(dispatcher.serve_one(&server_socket).is_err());
        assert_eq!(outcomes.lock().unwrap().as_slice(), &[ScanOutcome::Aborted]);
    }
