...
/// Attempts to create a subvolume. Returns `Ok(false)` if the operation
/// failed because the containing directory is not on Btrfs.
pub(super) fn create_btrfs_subvolume(path: &Path) -> Result<bool, CommandError> {
    let output = btrfs_command()
        ...
        .map_err(|err| {
            if err.kind() == io::ErrorKind::NotFound {
                user_error("Failed to create Btrfs subvolume: `btrfs` command is not installed")
            } else {
                user_error(format!("Failed to create Btrfs subvolume: {err}"))
            }
        })?;
    if !output.status.success() {
        ...
        return Err(user_error(format!(
            "Failed to create Btrfs subvolume: {stderr}"
        )));
    }
    Ok(true)
}
...
/// Deletes a previously verified Btrfs subvolume by stable subvolume ID.
///
/// `mount_path` only identifies the filesystem containing `subvolume_id`; the
/// mutable workspace pathname is deliberately not used to select the object.
pub(super) fn delete_btrfs_subvolume_by_id(
    subvolume_id: u64,
    mount_path: &Path,
) -> Result<(), CommandError> {
    let output = btrfs_command()
        .args(["subvolume", "delete", "--subvolid"])
        .arg(subvolume_id.to_string())
        .arg(mount_path)
        .output()
        .map_err(|err| {
            if err.kind() == io::ErrorKind::NotFound {
