...
use crate::commands::btrfs::btrfs_subvolume_id;
use crate::commands::btrfs::btrfs_user_subvol_rm_allowed;
use crate::commands::btrfs::delete_btrfs_subvolume_by_id;
...
pub async fn cmd_workspace_remove(
    ...
    let verified_subvolume_id = if is_btrfs_subvolume(&target.path).unwrap_or(false) {
        btrfs_subvolume_id(&target.path)?
    } else {
        None
    };
    #[cfg(feature = "git")]
    ...
    let deletion_result = match verified_subvolume_id {
        Some(subvolume_id) => {
            let mount_path = claimed.path.parent().unwrap_or(&claimed.path);
            delete_btrfs_subvolume_by_id(subvolume_id, mount_path).map_err(|mut err| {
                add_btrfs_delete_hint(&mut err, &claimed.path, subvolume_id);
                err
            })
        }
        None => delete_claimed_directory(&claimed.path, &target.identity),
    };
...
fn add_btrfs_delete_hint(err: &mut CommandError, path: &Path, subvolume_id: u64) {
    ...
    let mount_path = path.parent().unwrap_or(path);
    let display_path = mount_path.to_string_lossy();
    let quoted_path = shlex::try_quote(&display_path).unwrap_or_else(|_| display_path.clone());
    err.add_hint(format!(
        "{mount_hint} The workspace was not forgotten. Run `sudo btrfs subvolume delete \
         --subvolid {subvolume_id} {quoted_path}` only after deciding whether to forget its \
         registration."
    ));
}
...
fn preflight_workspace_removal(
    ...
    for (survivor_name, _) in workspace_command.repo().view().wc_commit_ids() {
        ...
        let survivor = resolve_registered_workspace(
            command,
            workspace_command,
            workspace_store,
